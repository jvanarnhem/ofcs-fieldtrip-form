 ...
 * For buildings set to 'digest' mode (Settings tab, super-admin only - see
 * SettingsHandlers.js), sendBuildingAdminNotification is skipped per-submission
 * (FormHandlers.js) and this rollup runs once a day instead.
 */

/**
 * Emails each digest-mode building admin a single rollup of everything still
 * awaiting their building-level approval. Run once a day via a time-based
 * trigger - see createDailyDigestTrigger() below for one-time setup.
 */
function sendDailyDigests() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (rowObj.status !== STATUS_VALUES.PENDING_BUILDING) continue;
