...
**Status: live in production as of 2026-08-16.** Deployed at the user's explicit go-ahead, redeployed by `ofcsdistrict@ofcs.net` (preserving the de-personalized execute-as identity — see "De-personalizing prod" below). Note that dev's own `_Settings`/`_Admins` values (lunch options, cost tiers, central email, kitchen staff roster) do **not** carry over automatically — those need real values entered directly in prod (via the Lunch Dashboard's own Settings tab and the `_Admins` roster), and `createLunchReminderTrigger()` needs a one-time manual run from prod's Apps Script editor, same as `createDailyDigestTrigger()`.
...
- **Important: `.clasp.prod.json`'s Apps Script project is the one serving real district staff right now** — live on this rewrite since 2026-08-09, operating against the production spreadsheet (`1uBP3wa_trcWF2-UpWsRQPkVMqZxkMyZQLUuw4Ot22T4`) via `APP_SPREADSHEET_ID` — see "Flipping the switch to production" below for how that happened, including the legacy rollback path. The bookmarked deployment (`AKfycbwvAfRXYmGgD4ehXch9HgF1MQ0ROx8vLkMKLbaTa4lBRUsJsb-f`) has been redeployed more than once since the original cutover (most recently to version 48, 2026-08-16, adding the Lunch Dashboard and the code-generated trip document — see CLAUDE.md's git history for what shipped when); don't trust a specific version number written here as current without checking `clasp deployments`. **Do not `clasp push` while pointed at prod casually** — `clasp push` replaces the entire remote file set, and the bookmarked deployment only picks up new code once someone explicitly runs `clasp create-version` + `clasp redeploy` against it (same two-step pattern used for the original flip), but pushing still overwrites what `@HEAD`/test deployments see immediately. Treat any prod push as a real production change now, not a dry run against a dormant project.
